...
def main():
	...
	class cards_:
		...
		def catbalou_panic(self, card_played, card_num, ps_blue_cards, ps_blue_cards_width, player_box):
			...
			for i in range(plnum):
				...
				for j in range(len(ps_blue_cards[i-1])):
					...
					event_rect.append([(ps_blue_cards[i-1][j][0],ps_blue_cards[i-1][j][1]),(ps_blue_cards[i-1][j][0]+ps_blue_cards_width[i-1][j],ps_blue_cards[i-1][j][1]+28)])
					pygame.draw.rect(screen, (150,0,0), rect, 2)
			pygame.display.update()
			
			items = len(ps_blue_cards)
			#blue_cards = [[me],[("colt"),(),()],[("colt")],[("colt"),()]]
			#                          your card box
			target_box = events(0, [((178,755),(1240,793))]+event_rect+player_box)
			#if you press "end turn"
			print("taget boc:", target_box)
			...
			# if you select another card
				...
				message("You can't use your %s on their Colt .45!"%card_played)			
				return ('b', blue_cards)									
			...
			if target_box < items:
				...
				#target box = 0
				#(0,1,2)("3")(4,5)(6)
				temp = target_box+len(blue_cards[0])
				if b_c_unnested[temp][0] != "Colt .45":
					if card_played == "Cat Balou":
						discard.append(b_c_unnested[temp])
					elif card_played == "Panic!":
						in_hand[0].append(b_c_unnested[temp])
					del b_c_unnested[temp]
		...
		def jail(self, card_num=None, target=None):
			...
			if "Jail" in [*map(itemgetter(0), blue_cards[turn])]:
				if draw(False, True) == "h":
					turn += 1
					message("%s missed their turn!"%game_char[1][turn][0])
					return 1

		def beer(self, card_num, lives_box, lives):
			if lives[turn] != game_char[1][turn][0]:
				self.pre_card(card_num)
				lives[turn] += 1
		...
		def indians_gatling(card_num, card_played, lives):
			...
			for target in range(plnum):
				if target == turn:
					continue
				if game_char[1][target][1] == "Calamity Janet":
					num = char.calamityjanet()
					if num == 0:
						return
				cards_in_targets_hand = [*map(itemgetter(0), in_hand[target])]
	...
	def preparation():
		...
		"""
		outputlist = []
		for values in game_char[1]:
    		outputlist.append(values[0])
    	"""
    	outputlist = [values[0] for values in game_char[1]]

    	global your_turn

    	p_lives_box = (1152,704,50,60)
		...
		for i in range(max(outputlist)):
			...
			for j in range(plnum):
				...
				#print(i,j, game_char[1][j][0])
				# i % plnum
				if (i < game_char[1][j][0]):
					#last time, eg/ i=4
					cards.give_card(j)
		#print(in_hand)
		...
		screen.blit((gamefont.render("Character:           %s"%(game_char[1][0][1]),0,(0,0,0))), (17,670))
		screen.blit((gamefont.render("Ability:                %s"%(game_char[1][0][2]),0,(0,0,0))), (17,700))
		screen.blit((gamefont.render("Role:                    %s"%(game_char[0][0]),0,(0,0,0))), (17,730))
		...
		use_card = [use_card_text, use_card_rect]
		#print(use_card)

		return (lives, start_turn_text, start_turn_x, start_turn_y, start_turn_rect, cards_rect, blue_card_rect, shot_text, itemed_text, blue_cards, p_lives_box, use_card)

	def death(lives, target):
		global plnum 
		if game_char[0][target] == 'Outlaw':
			death_text = 'An outlaw died!'
			#death_text_pos = (548, 522)
			#get 3 cards
			for i in range(3):
				cards.give_card(turn)
		elif game_char[0][target] == 'Renegade':
			death_text = 'The renegade died!'
			#death_text_pos = (530.5, 522)
		elif game_char[0][target] == 'Deputy':
			death_text = 'A deputy died!'
			#death_text_pos = (551.5, 522)
			#if the sheriff killed the deputy
			if game_char[0][turn] == "Sheriff":
		...
		for i in range(plnum):
			if game_char[1][i][1] == "Vulture Sam":
		...
		del game_char[0][target]
		del game_char[1][target]
		del lives[target]

		#add victory conditions
		if "Sheriff" not in game_char[0]:
			#outlaws in
			pass
		elif "Outlaw" not in game_char[0] and "Renegade" not in game_char[0]:
			#sheriff and deputy win
			pass
		elif plnum == 1 and "Renegade" in game_char[0]:
			#renegade wins
			pass
	...
	def player_spots(lives):
		...
		for index in range(plnum):
			name_display[index] = gamefont.render(game_char[1][index][1], 0, (0,0,0))
		...
		for i in range(plnum):
			if i == 0:
				...
				name_text[i-1] = gamefont.render((str(lives[i]) + " " + game_char[1][i][1] + " " + str(len(in_hand[i]))),0,(0,0,0))
	...
	def game():
		...
		#time.sleep(0.25)
		global blue_cards
		(lives, start_turn_text, start_turn_x, start_turn_y, start_turn_rect, cards_rect, blue_card_rect, shot_text, itemed_text, blue_cards, p_lives_box, use_card) = preparation()
		...
		while running:	
			...
			global turn
			#sheriff goes first
			#turn = (game_char[0]).index("Sheriff")
			turn = 0
			#your turn
			if turn == 0:
				...
				while your_turn == True:
					...
					try:
						...
						pass
					#gets rid of extra red lines around the player
					for i in range(plnum-1):
						pygame.draw.rect(screen, background, (player_orientation[plnum-2][i][0], player_orientation[plnum-2][i][1], 200, 40))
					click =  p1_cards_display(cards_rect)
					(player_orientation, name_text, player_spots_blue_cards, player_spots_blue_cards_width) = player_spots(lives)
					pygame.display.update()
					...
					if card_played in cards.bl_cards:
						...
						blue_box = events(0 , [((15,620),(1235,645))]+[((cards_rect[0],cards_rect[1]),(cards_rect[0]+cards_rect[2],cards_rect[1]+cards_rect[3]))])
						#if you press "end turn"
					...
					#["Bang!", "Missed!", "Beer", "Duel", "Cat Balou", "Panic!", "Indians!", "Gatling", "Saloon", "General Store", "Stagecoach", "Wells Fargo"]
						...
						if card_played in ['Bang!', 'Duel', 'Cat Balou', 'Panic!', 'Jail']:
							...
							for i in range(plnum):
								...
								red_player_box[i-1] = ((red_player_rect[i-1][0],red_player_rect[i-1][1]),(red_player_rect[i-1][0]+red_player_rect[i-1][2],red_player_rect[i-1][1]+red_player_rect[i-1][3]))
							...
							if card_played == 'Cat Balou' or card_played == 'Panic!':
								...
								(num, blue_cards) = cards.catbalou_panic(card_played, card_num, player_spots_blue_cards, player_spots_blue_cards_width, red_player_box)
								#print(num)
								#gets rid of red lines
								for i in range(plnum-1):
									#pygame.draw.rect(screen, background, red_player_rect[i], 2)
									pygame.draw.rect(screen, background, (player_orientation[plnum-2][i][0]-2, player_orientation[plnum-2][i][1]-22, 200, 50+27*len(blue_cards[i])))		
							...
							#[ ((#,#),(#,#)), ((#,#),(#,#)), ]
							target = events(0, [((cards_rect[0],cards_rect[1]),(cards_rect[0]+cards_rect[2],cards_rect[1]+cards_rect[3]))]+red_player_box)
							#if you press "end turn"
							if type(target) == str:
								print("end turn")
								break
							# if you select another card
								...
								if (card_played == 'Bang!'):
									...
									#print("Willy? ", game_char[1][0][1],(game_char[1][0][1] == 'Willy The Kid'))
									#print("Bang played? ", bang_played != 0)
									if (bang_played == 0 or ((game_char[1][0][1] == 'Willy The Kid') or (blue_cards[0][0][0] == 'Volcanic'))):
										print("bang was played")
										num = cards.bang(target, card_num, lives)
										bang_played = 1
										#if death occurs
										if num == 1:
											death(lives, target)
										...
										# and not ((game_char[1][0][1] == 'Willy The Kid') or (in_hand[0][0][0] == 'Volcanic')):
						...
						#cards used on yourself	
							...
							your_name = events(0, [((cards_rect[0],cards_rect[1]),(cards_rect[0]+cards_rect[2],cards_rect[1]+cards_rect[3]))]+[((184,672),(334,697))])
							#if you click end turn
							if  type(your_name) == str:
								...
								if card_played == 'Beer':
									cards.beer(card_num, p_lives_box, lives)
						...
						#untargeted cards
							...
							screen.blit(use_card[0], (use_card[1][0],use_card[1][1]))
							pygame.draw.rect(screen, (150,0,0), use_card[1], 2)
							pygame.display.update(use_card[1])

							use_card_event = events(0, [((cards_rect[0],cards_rect[1]),(cards_rect[0]+cards_rect[2],cards_rect[1]+cards_rect[3]))]+[((use_card[1][:2]),(use_card[1][0]+use_card[1][2],use_card[1][1]+use_card[1][3]))])
							#if you click end turn
